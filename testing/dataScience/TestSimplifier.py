...
class TestSimplifier(unittest.TestCase):
    ...
    def test_get_list_cwi_predictions(self):
        s = Simplifier()
        pred = s.get_list_cwi_predictions(
            "The categories of occupations or undertakings in respect of which the Member "
            "proposes to have recourse to the provisions of paragraph 1 of this Article "
            "shall be specified in the declaration accompanying its ratification.")
        print(pred)
        self.assertEqual(pred, [False, False, False, True, False, True, False, False, False, False, False, False, True, False, False, True, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, True, False, True, True])
    ...
    def test_get_bert_candidates(self):
        ...
        result = s.get_bert_candidates(sentence,s.get_list_cwi_predictions(sentence))
        print(result)
    ...
    def test_get_syntactically_simplified_text(self):
        s = Simplifier()
        result = s.get_syntactically_simplified_text('''No person shall be held guilty of an offence on account of any act or omission which did not, at the time of such act or omission, constitute such an offence and no penalty shall be imposed for any offence more severe than the penalty in force at the time such offence was committed.''')
        print(result)
        self.assertEqual(result,result)
    ...
    def test_sentences_list(self):
        s = Simplifier()
        result= s.sentences_list(['This', 'Ordinance', 'may', 'be', 'cited', 'as', 'the', 'Brothels', 'Ordinance'],['This', 'Ordinance', 'may', 'be', 'cited', 'as', 'the', 'Brothels', 'Ordinance'])
        print(result)
        self.assertEqual(result, ['This Ordinance may be cited as the Brothels Ordinance', 'This Ordinance may be cited as the Brothels Ordinance'])
